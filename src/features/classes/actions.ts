...
export async function createClassAction(
  ...
  const { data, error } = await supabase.rpc("create_class", { p_name: parsed.data.name });
...
export async function joinClassAction(
  ...
  const supabase = await createClient();
  const { data, error } = await supabase.rpc("join_class_by_code", { p_code: parsed.data.code });
...
/** Quitte une classe (perd l'accès à ses cours, sauf adhésion explicite). */
